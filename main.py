...
def coz():
    ...
    try:
        ...
        if post.is_video:
            ...
            # display_resources içinden en yüksek çözünürlüklü olanı seçer
...
def try_alternative_method(url):
    try:
        ...
        # Resim - JSON içindeki display_resources veya display_url taraması (Daha yüksek kalite)
        display_resources = re.findall(r'"display_url":"([^"]+)"', response.text)
        if display_resources:
            # Genelde listenin sonundaki link en yüksek çözünürlüklüdür
